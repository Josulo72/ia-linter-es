...
2. El análisis no usa red, LLM ni GPU. Ningún módulo de `src/` importa `http`, `https`, `net`, `dns`, `child_process` ni `worker_threads`. Solo `runner/`, `cli/`, `config/`, `baseline/`, `rules/compiler/` y `api/` pueden importar `fs` (la API desde el 2026-09-18, ver `docs/decisions.md`).
